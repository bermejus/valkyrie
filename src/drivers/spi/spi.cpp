...
{
    p_spi->SPI_MR &= (~SPI_MR_PCS_Msk);
    p_spi->SPI_MR |= SPI_MR_PCS(spi_get_pcs(ul_value));
}
...
void spi_send_cmd(Spi* p_spi, spi_cmd_t* cmd)
{
    ...
    {
        spi_write(p_spi, cmd->cmd[i], 0, 0);
        spi_read(p_spi, &data, &pcs);
        if ((i == (cmd->cmd_size - 1)) &&
                    (cmd->dummy_size == 0) &&
                    (cmd->data_size == 0))
    ...
    for (int i = 0; i < cmd->dummy_size; i++)
    {
        spi_write(p_spi, 0xFF, 0, 0);
        spi_read(p_spi, &data, &pcs);
    }
    ...
    if (cmd->rx)
    {
        for (int i = 0; i < cmd->data_size; i++)
        {
            spi_write(p_spi, 0xFF, 0, 0);
            spi_read(p_spi, &data, &pcs);
            if (i == (cmd->data_size - 1))
                spi_set_lastxfer(p_spi);
            cmd->data[i] = (uint8_t)(data & 0xFF);
        }
    }
    else
    {
        for (int i = 0; i < cmd->data_size; i++)
        {
            spi_write(p_spi, cmd->data[i], 0, 0);
            spi_read(p_spi, &data, &pcs);
            if (i == (cmd->data_size - 1))
                spi_set_lastxfer(p_spi);
        }
...
/*
    * Configure CS behavior for SPI transfer (\ref spi_cs_behavior_t).
    *
    * \param p_spi Pointer to an SPI instance.
    * \param ul_pcs_ch Peripheral Chip Select channel (0~3).
