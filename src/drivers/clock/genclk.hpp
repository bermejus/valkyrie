...
//! Programmable Clock Sources (PCK)
enum genclk_source {
...
static inline void genclk_config_defaults(genclk_config *p_cfg,
        uint32_t ul_id)
{
    ul_id = ul_id;
    p_cfg->ctrl = 0;
...
static inline void genclk_config_read(genclk_config *p_cfg,
        uint32_t ul_id)
{
    p_cfg->ctrl = PMC->PMC_PCK[ul_id];
}

static inline void genclk_config_write(const genclk_config *p_cfg,
        uint32_t ul_id)
{
...
static inline void genclk_config_set_source(genclk_config *p_cfg,
...
static inline void genclk_config_set_divider(genclk_config *p_cfg,
        uint32_t e_divider)
{
...
static inline void genclk_enable(const genclk_config *p_cfg, uint32_t ul_id)
{
    PMC->PMC_PCK[ul_id] = p_cfg->ctrl;
...
static inline void genclk_enable_source(genclk_source e_src)
{
    switch (e_src) {
    case GENCLK_PCK_SRC_SLCK_RC:
...
static inline void genclk_enable_config(unsigned int id, genclk_source src, unsigned int divider)
{
